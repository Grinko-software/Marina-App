    ...
    persist(
        (set) => ({
            ...
            handleProductRequest: (withImage, lastListInventory) => {
                ...
                try {
                    getData(
                        withImage ? PRODUCT_API_URL + '?include=image' : PRODUCT_API_URL
                    ).then((result) => {
                        if (result?.code === 200) {
                            set({
                                listInventory: result?.data
                                    ?.reduce(
                                        (
                                            acc,
                                            {
                    ...
                    getData(CATEGORIES_API_URL, GET).then((response) => {
                        ...
                        if (response?.code === 200) {
                            const data = response?.data?.reduce((acc, value) => {
                                return [
                                    ...acc,
                                    { id: value?.ID, label: value?.name.toUpperCase() }
                                ]
                            }, [])
                            set({ listCategories: data })
                        } else {
                            notify(
                                '❌No se pudieron obtener las categorías, reintente nuevamente!'
                            )
            ...
            getProductByCode: (products, searchCode) => {
                ...
                if (!result) {
                    ...
                    result = products?.find(
                        ({ code }) =>
                            firstChar !== code[0] &&
							searchCode.includes(code) &&
							Math.abs(code?.length - codeSearchSize) === 1
                    )
                }
