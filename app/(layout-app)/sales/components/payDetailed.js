...
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, Input, Text, Textarea } from '@nextui-org/react'
...
export default function PayDetailed ({ payment, setPageTarget, setPayment, isOpen, onClose, setGoPay, totalPay, payDetailed, setPayDetailed, listSales, createSale, paymentTarget, voucherTarget, clearList, pageTarget, onOpen, setPaymentTarget, setSearchInput, setVoucherTargetValue, onOpenLoadingSale }) {
    ...
    const handleCancelSale = async () => {
        ...
        // const response = await cancelSale({ detail: detailCancel, notify })
        setOpenModalCancelSale(!openModalCancelSale)
    ...
    return (
        <section>
            <Modal
                size='5xl'
                className={`${openModalCancelSale ? '' : 'h-[40rem]'}`}
                isOpen={isOpen}
                backdrop='opaque'
            ...
            >
                ...
                { openModalCancelSale ? <ModalContent className='items-center content-center py-10' >
                    <ModalHeader className="flex flex-col text-primary-500 dark:text-primary-200  text-2xl font-extrabold ">
                       Venta cancelada
                    </ModalHeader>
                    ...
                    <ModalFooter className='justify-center'>
                        <div className='space-x-3'>
                            <Button variant="shadow" className="w-[18rem] h-[6rem] text-2xl font-extrabold text-primary-50"
                                isDisabled={loadingSale}
                                onClick={() => {
                                    // Agregar pk cancela..
                                    setOpenModalCancelSale(false)
                                }}
                            >
                            Volver
                            </Button>
