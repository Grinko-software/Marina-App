...
export default function SaleList (props) {
    ...
    const [loadingHandleButtonClick, setLoadingHandleButtonClick] =
		useState(false)
                ...
                <section className="flex flex-row px-1">
                    ...
                    <div className="flex flex-col m-3 space-y-1">
                        <Button
                            size="sm"
                            color="success"
                            className="font-bold text-lg h-[1.6rem] text-white"
                    ...
                    {listSales.length > 0
                        ? (
                            <div className="flex items-center justify-between px-3">
                                <h5 className=" animation-fade-in text-2xl font-bold leading-none text-gray-900 dark:text-white pt-2">
								Productos
                                </h5>
                                <Button
                        ...
                        : (
                            <h5 className="animation-fade-in m-5 text-2xl font-bold leading-none text-gray-900 dark:text-white">
                                {loading
                                    ? 'Espere un momento...'
                                    : 'Escanee un producto para comenzar la venta...'}
                            </h5>
                        )}
                </section>
                        ...
                        >
                            {listSales?.map((product, index) => (
                                ...
                                >
                                    ...
                                    <SaleListItem
                                        deleteEnable={listSales?.length > 1}
                                        product={product}
                                        viewPaymentOpen={payment}
                                    />
                                    <Divider orientation="horizontal" />
                                </section>
                            ))}
                                    ...
                                    >
                                        <div className="flex flex-row gap-1 items-center">
                                            <p className="text-primary-50">
                                                {totalPrice
                                                    ? formatter.format(transformNumberFormat(totalPrice))
                                                    : null}
                                            </p>
                                            <p className="text-red-600">
                                                {discount
                                                    ? ' - ' +
												  formatter.format(transformNumberFormat(discount))
                                                    : null}
                                            </p>
                                        </div>
                                        <p className="text-primary-50">
                                            ...
                                            {formatter.format(
                                                transformNumberFormat(totalPrice - discount)
                                            )}
                                        </p>
                                    </motion.div>
                                </div>
