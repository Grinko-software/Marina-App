...
export default function WeighingScaleModal({ isOpen, onClose, product }) {
	...
	const { addFromNewSales, listSalesActives, saleIdActive } = useSalesStore();
	...
	useEffect(() => {
		if (isDefaultValue) {
			onSubmitHandler(
				product,
				valueKg,
				Math.round((product?.price * valueKg) / 10) * 10
			);
		}
			...
			<Modal backdrop="blur" isOpen={isOpen} onClose={onClose} size={'2xl'}>
				<ModalContent className="">
					<ModalHeader className="flex flex-col gap-1 text-primary-500 dark:text-primary-200 ">
						...
						<Button
							variant="shadow"
							className="w-full text-white font-extrabold tracking-widest bg-emerald-600"
							onClick={() => {
								setManualMode(!manualMode);
							}}
						>
							{manualMode ? 'MODO AUTOMATICO' : 'MODO MANUAL'}
						</Button>
					</ModalHeader>
					<ModalBody className="flex flex-col ">
						<Card isBlurred className="border-none w-full" shadow="sm">
							<CardBody>
								<div className="flex flex-row gap-10   content-between">
									<div className="col-span-6 md:col-span-4 dark:bg-default-100/50 bg-default-100/70 rounded-xl">
										...
										<div className="m-5 mt-5">
											...
											<div className="flex felx-row gap-5">
												...
												<h1 className="text-md text-lime-500 ">
													${product?.price}
												</h1>
											</div>
										</div>
									...
									<section className="flex flex-col items-center justify-center content-between gap-2 h-[20rem]">
										{!manualMode ? (
											<div className="flex flex-col gap-14 ">
												...
												<div className="flex flex-row justify-center ">
													<h1 className="text-9xl  text-teal-500 font-medium ">
														{valueKg}
													</h1>
													<h1 className="text-lg text-zinc-400 font-medium ">
														KG
													</h1>
												</div>
											</div>
											...
											<div className="w-full">
												<div
													key="flat"
													className="flex h-[4rem] flex-wrap md:flex-nowrap mb-4 md:mb-0 gap-2"
												>
													<Input
														type="number"
														variant={'flat'}
														label="KG"
														placeholder="Ingresa manualmente los KG."
														className="text-sm"
														onValueChange={(value) => setValueKg(value)}
														value={valueKg}
													/>
												</div>
												<div className="flex flex-col gap-1 w-full">
													<Button
														className="w-full h-[3rem] bg-green-600 text-white font-bold text-lg"
														onClick={() => {
															setValueKg(1);
										...
										<div className="flex flex-row w-[20rem] gap-4 items-center px-5 py-3 border-3 border-teal-500 rounded-xl text-teal-500">
											<h1 className="text-3xl w-full  text-teal-500 font-bold">
												TOTAL
											</h1>
											<h1 className="text-3xl  w-full font-xl font-bold text-teal-500 ">
												$ {Math.round((product?.price * valueKg) / 10) * 10}
											</h1>
										</div>
									</section>
					...
					<ModalFooter>
						<section className="flex flex-row w-full justify-center">
							<Button
								color="success"
								variant="shadow"
								className="w-full font-extrabold text-5xl text-white h-[6rem]"
								onClick={() =>
									onSubmitHandler(
										product,
										valueKg,
