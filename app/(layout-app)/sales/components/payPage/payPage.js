...
export default function PayPage (props) {
    ...
    const { listInventory } = useInventoryStore(({ listInventory }) => ({
        listInventory
    }))
        ...
        <section className="animation-fade-in h-full w-full p-2">
            ...
            <section className="z-10 h-[3.5rem] w-[7rem] rounded-t-[12px] bg-secondary-50 dark:bg-secondary-450">
                <Button
                    isDisabled={true}
                    size="lg"
                    className="flex flex-col items-center h-full w-full font-bold"
                    isIconOnly
                    variant="ligth"
                    aria-label=""
                    onClick={() => {
            ...
            <section className="flex flex-col h-3/4 mt-[-0.2rem] sm:h-[93%] items-center px-5 shadow-md hover:shadow-lg  rounded-tl-[0px]  bg-secondary-50 dark:bg-secondary-450 rounded-[14px]">
                <div className="flex flex-col w-full h-full">
                    <div className="flex flex-row py-[0.7rem] items-end justify-end gap-5">
                        ...
                        <div className="">
                            <Button
                                className="animation-fade-in h-[56px] text-lg  bg-amber-400 dark:bg-amber-400 font-semibold"
                                color="danger"
                                variant="bordered"
                    ...
                    <h5 className="text-4xl font-bold leading-none text-gray-900 dark:text-white">
						Seleccione Boleta, factura o Ticket
                    </h5>
                            ...
                            {loading
                                ? (
                                    <section className="flex flex-row space-x-10">
                                        {listEmpty3?.map((element, index) => (
                                            <Card
                                                key={index}
                                                className="animation-fade-in p-1 w-[176px] h-[176px]"
                                                shadow="sm"
                                            >
                ...
                <div className="flex flex-col  w-full h-full mt-16 items-center">
                    <h5 className="text-4xl font-bold leading-none text-gray-900 dark:text-white flex-initial">
                    ...
                    <div className="flex flex-col items-center">
                        <div className="flex flex-row  space-x-10 mt-10 item-center">
                            {loading
                                ...
                                : (
                                    listPayments?.filter((pay) => pay?.id !== 7)?.map((pay) => (// TODO: FILTRO PAGO MIXTO
                                        <PaymentButton
                                            key={pay?.id}
                                            id={pay?.id}
                                            icon={
