...
export default function ListTask({ taskDifficulties, taskStates }) {
	...
	return (
		<div className="flex flex-col items-start sm:p-6 w-full xl:max-w-[1200px] mx-auto h-full flex-1">
			{loading ? (
				<div className="h-full w-full flex flex-col justify-center items-center">
					<Spinner color="success" size="lg" label="Cargando tareas ..." />
				</div>
			) : (
				<div className="w-full flex flex-col gap-4 flex-1 h-full">
					{hasGeneralTasks && (
						<div className="w-full flex flex-col divide-y divide-gray-300 flex-1 h-full">
							{[
								{
								...
								<div key={item.title} className="w-full flex-1 h-full">
									<h2 className="text-lg sm:text-xl font-bold mb-2 text-center">
										{item.title}
									</h2>
									<div
										className={`w-full ${
											item.title === 'Actividades por hacer'
