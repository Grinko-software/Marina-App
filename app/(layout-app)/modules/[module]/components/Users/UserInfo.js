...
import {
    createUserAssociationCredential,
    deleteUserAssociationCredential
...
export default function UserInfo (params) {
    ...
    const handleGetUserPermission = () => {
        getUserPermission({ id: target?.id }).then((modules) => {
    ...
    const handleResetPassword = () => {
        if (target?.id != null) {
            requestResetPassword({
                id: target?.id,
                notify,
                onSuccess: closeModalWithRefresh
            })
            handleRefresh()
        }
    }
    ...
    const addModuleUser = (moduleKey) => {
        ...
        const hasModule = [...currentModules].find(
    ...
    const handleUpdateUserPermission = async ({
        ...
        const modulesToDelete = currentModules.filter(
            (module) =>
                !updatedModules.some(
                    (updatedModule) => updatedModule?.moduleKey === module?.moduleKey
                )
        )
        ...
        const modulesToAdd = updatedModules.filter(
            (module) =>
                !currentModules.some(
                    (currentModule) => currentModule?.moduleKey === module?.moduleKey
                )
        )
    ...
    const handleUpdateUser = async () => {
        ...
        if (userDataUpdated) {
            const { code: credentialCodeUpdated, ...restDataUpdated } =
				userDataUpdated

            if (
                credentialCodeUpdated !== undefined &&
				credentialCodeUpdated !== target?.credential?.code
            ) {
                if (userDataUpdated.code) {
                    await createUserAssociationCredential({
                        userId: target?.id,
                        keyCredential: credentialCodeUpdated,
                        notify
                    })
                } else {
            ...
            if (Object.keys(restDataUpdated).length) {
                await requestUpdateUser({
                    id: target?.id,
                    name: restDataUpdated?.name,
                    email: restDataUpdated?.email,
                    lastName: restDataUpdated?.lastName,
                    password: restDataUpdated?.password,
                    notify,
                    onSuccess: closeModalWithRefresh
                })
            } else {
    ...
    const handleDeleteUser = () => {
        ...
        deleteUser({ id: target.id, notify }).then((response) => {
                    ...
                    <ModalBody>
                        <section className="items-start gap-2 grid grid-cols-1">
                            <section className="items-center gap-2 grid grid-cols-1 md:grid-cols-2 mb-5">
                                <div className="px-4 flex items-center">
                                    <Input
                                        ...
                                        onValueChange={(value) => {
                                            handleUpdateUserValues({ name: value })
                                        }}
                                    />
                                </div>
                                <div className="px-4 pt-4 flex items-center">
                                    <Input
                                        ...
                                        onValueChange={(value) => {
                                            handleUpdateUserValues({ lastName: value })
                                        }}
                                    />
                                </div>
                                <div className="px-4 pt-4  flex items-center">
                                    <Input
                                        ...
                                        onValueChange={(value) => {
                                            handleUpdateUserValues({ email: value })
                                        }}
                                    />
                                </div>
                                {/*  <div className="px-4 pt-4  flex items-center">
                                ...
                                <div className="px-4 pt-4  flex items-center">
                                    <Input
                                        ...
                                        onValueChange={(value) => {
                                            handleUpdateUserValues({ password: value })
                                        }}
                                        endContent={
                                            <button
                            ...
                            <section className="mx-[1rem] gap-4 flex flex-col mb-5">
                                <div>
									Módulos asociados {userModules?.length}/{allModules?.length}
                                </div>

                                <div className="flex flex-wrap flex-row gap-1">
                                    {allModules?.map((module) => {
                                        const isActive = hasModuleActive({
                                            modules: edit ? userModulesUpdated : userModules,
                                            ...
                                            <div
                                                ...
                                                onClick={() =>
                                                    !edit
                                                        ? undefined
                                                        : isActive
                                                            ? deleteModuleUser(module.moduleKey)
                                                            : addModuleUser(module.moduleKey)
                                                }
                                                className=""
                                            >
                                                <Chip
                                                ...
                                                >
                                                    <div className="text-md">{module.moduleName}</div>
                                                </Chip>
                                            </div>
                                        )
                                    })}
                            ...
                            <section className="mx-[1rem] gap-4 flex flex-col">
                                ...
                                <UserCredential
                                    ...
                                    onValueChage={onChangeCredentialCode}
                                />
                            </section>
                        </section>
                    </ModalBody>
