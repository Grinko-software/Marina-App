...
export default function UserCredential({ credential, isEdit, onValueChage }) {
	...
	const {
		requestData: requestDataCredentials,
		associationData,
		data: credentialData
	} = useCredentialStore();
	...
	useEffect(() => {
		const credentialsModelData = associationData?.map(
			({ ID, key_code, key_name, is_associated, user_association_id }) => {
				return {
					id: ID,
					name: key_name,
					code: key_code,
	...
	const downloadQRCode = (componentRef, code) => {
		toPng(componentRef.current).then(function (dataUrl) {
	...
	return (
		<section
			className={`w-full items-center gap-2 grid grid-cols-1 ${
				isEdit && credential ? 'md:grid-cols-2' : ''
			}`}
		>
			{credential ? (
				<div className="flex flex-row m-auto gap-5">
					<div className="flex h-full flex-col items-end my-auto">
						...
						<p className="text-md font-semibold">
							{credential?.name?.toUpperCase() || 'Sin nombre'}
						</p>
					</div>
					<Divider orientation="vertical" className="h-[1-rem] w-[2px]" />
					<div className="flex flex-col gap-5">
						<div
							className="h-auto gap-2 max-w-60 border mr-auto bg-white rounded-xl"
							ref={componentRef}
						>
							<QRCode
								...
								icon={
									downloadQR
										? null
										: 'https://i.pinimg.com/originals/f5/c4/3d/f5c43df87ed342297a519ba9d202e111.png'
								}
							/>
						</div>
					...
					>
						{(item) => (
							<AutocompleteItem key={item.value}>
								{`${item.label} ${
									currentCredentialCode === item.code
										? '(ACTUAL)'
										: item.isAssociated
										? '(ASOCIADO)'
										: ''
