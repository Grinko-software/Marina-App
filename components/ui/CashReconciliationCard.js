...
export default function CashReconciliationCard (props) {
    ...
    return (
        <>
            ...
            <Card className="col-span-12 sm:col-span-4 h-[400px] w-[400px] bg-yellow-400 ">
                <CardHeader className={`${bgTitle} bottom-0 border-1 rounded-md border-zinc-100/50 z-10 !items-start h-[80px]`}>
                    <div className="grid grid-rows-2 content-between h-[120px]">
                        <p className=" text-white/80 uppercase font-bold text-xl">{title}</p>
                    </div>
                </CardHeader>
                <h4 className="z-10 text-white/90 text-4xl font-bold px-2 my-2">{total}</h4>
                <Image
                    width={270}
                    removeWrapper
                    alt="Card background"
                    className="z-0 blur-sm object-bottom relative -mt-8"
                    src={img}
                />
