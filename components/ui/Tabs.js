...
import {
    /* Dropdown, DropdownItem, DropdownMenu, DropdownTrigger, */ Button,
    Card
    ...
    useEffect(() => {
        ...
        if (isClient && items?.length) {
            if (!selectedKey) {
                ...
                itemSelected = items?.find(
                    (item) => item?.id?.toString() === selectedKey
                )
            }
                ...
                ? (
                    <section className="flex flex-auto flex-col sm:flex-row">
                        ...
                        <div className="z-10 ml-0 sm:ml-[1rem]">
                            <div
                                className={
                                    'w-[48vh] sm:w-[36rem] p-0 rounded-2xl border-0 overflow-visible z-10 absolute  ' +
								(!openMenu ? 'hidden' : '')
                                }
                            >
                                <Card className=" z-10 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 sm:p-5 p-1 border-2 border-primary-300 dark:border-primary-400">
                                    {itemsTabs
                                        ...
                                        .map(({ label, id }) => {
                                            ...
                                            return (
                                                <div href="#" key={label} className="z-10">
                                                    <Button
                                                        className={`w-full sm:text-base text-[10px] ${
                                                            selected ? 'text-primary-50 bg-green-400' : ''
                                                        }`}
                                                        onClick={() => {
                                                            setOpenMenu(false)
                                                            setTimeout(() => {
