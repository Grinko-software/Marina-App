...
import {
	GET_INDICATORS_LAST_BALANCE,
	CREATE_BALANCE_BEGINNINGS,
	CREATE_BALANCE_ENDINGS,
	GET_INDICATORS_BALANCE_ENDING,
	CREATE_BALANCE_WITH_DRAWALS,
	OPEN_DRAWER_API_URL
} from '@/settings/constants';
const useCashBalanceStore = create((set) => ({
	error: null,
	loading: false,
	getLastIndicatorsCashBalanceEnding: (id, setLastBalance, onHandleState) => {
		set({ loading: true });
		try {
			getData(GET_INDICATORS_LAST_BALANCE.replace(':id', id), GET).then(
				(result) => {
					set({ loading: false });
					if (result?.code === 200) {
						onHandleState();
	...
	getIndicatorsBalanceEnding: (
		...
		set({ loading: true });
		try {
			getData(GET_INDICATORS_BALANCE_ENDING.replace(':id', id), GET).then(
				(result) => {
					set({ loading: false });
					if (result?.code === 200) {
						handlerOpenDrawer();
		...
		try {
			getData(CREATE_BALANCE_BEGINNINGS, POST, body).then((result) => {
				...
				if (result?.code === 200) {
					...
					notify(
						'❌ Primero se debe efectuar el cierre de la caja Nº ' +
							cashRegisterId +
							', antes de efectuar el inicio de caja'
					);
				} else {
					setStatusCashRegister(false);
	...
	createBalanceEndings: (
		...
		userId,
		detail,
		totalEndingRealCashBalance,
		totalEndingNominalCashBalance,
		totalEndingCard,
		setStatusCashRegister,
		onhandlerAcctions,
		notify
		...
		try {
			getData(CREATE_BALANCE_ENDINGS, POST, body).then((result) => {
				...
				if (result?.code === 200) {
					...
					setStatusCashRegister(false);
					notify(
						'❌ Primero se debe efectuar el incio de caja Nº ' +
							cashRegisterId +
							', antes de efectuar un cierre de caja.'
					);
				} else {
					setStatusCashRegister(false);
					notify('❌ ' + result?.message);
		...
		try {
			getData(CREATE_BALANCE_WITH_DRAWALS, POST, body).then((result) => {
				...
				if (result?.code === 200) {
					...
					if (eventType === 'income') {
						notify(
							'✅ Ingreso de efectivo a Caja Nº ' +
								cashRegisterId +
								' con éxito!'
						);
					} else {
						notify(
							'✅ Retiro de efectivo Caja Nº ' + cashRegisterId + ' con éxito!'
						);
					}
					handlerOpenDrawer();
				} else {
