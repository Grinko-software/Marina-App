...
export default function SettingsNav ({ isMobile }) {
    ...
    const { fullName, isAdmin } = useAuthStore(({ fullName, isAdmin }) => ({
        fullName,
        isAdmin
    }))
    ...
    useEffect(() => {
        if (
            selectedCashRegister?.ID !== 'no-select' &&
                ...
                <CardHeader className="justify-between space-x-2">
                    ...
                    >
                        {usePathname() === '/home'
                            ? (
                                <div>
                                    <Image
                                        className="w-16 h-16 p-1 rounded-full ring-4 ring-amber-600"
                                        src={imgSrc}
                                        alt="Bordered avatar"
                                    />
                                </div>
                        ...
                        <div className="flex flex-col items-start justify-center">
                            <h4 className="sm:text-xl  text-tiny font-semibold leading-none text-default-600">
                                {userName?.toUpperCase()}
                            </h4>
                            <h5 className="sm:text-xl text-tiny tracking-tight text-default-400">
                                {admin ? 'Administrador' : 'Trabajador'}
                            </h5>
                        </div>
                    ...
                    {usePathname() === '/home'
                        ? (
                            <Button
                                className={
                                    'text-xs sm:text-lg bg-transparent text-foreground border-default-200'
                                }
                                color="primary"
                                radius="full"
                                variant={'solid'}
