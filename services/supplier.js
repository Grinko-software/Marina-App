...
export const fetchCreateSupplier = async ({
    name,
    rut,
    companyName,
    companyRut
...
export const fetchUpdatedAssociationSupplier = async ({ supplierId, ids }) => {
    try {
        return await fetch(
            `${SUPPLIER_ASSOCIATION_API_URL.replace(':id', supplierId)}`,
            {
                method: 'POST',
                headers: new Headers({
                    Authorization: 'Bearer ' + getToken()
        ...
        return getData(
            ...
            try {
                if (response?.code === 200) {
                    ...
                    notify(
                        '❌ El proveedor no se pudo eliminar correctamente, intente mas tarde.'
                    )
                }
            } catch {
                return null
